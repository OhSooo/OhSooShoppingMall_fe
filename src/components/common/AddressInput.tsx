...
export default function AddressInput({
  ...
  const handleShippingAddressDetailChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    onChange({ ...value, shippingAddressDetail: e.target.value });
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      {label && (
        <label style={{ fontSize: '14px', fontWeight: '500', color: 'var(--color-gray-5)' }}>
          {label} {required && <span style={{ color: 'var(--color-point-main)' }}>*</span>}
        </label>
      )}
      ...
      {(error || shippingAddressDetailError) && (
        <div style={{ fontSize: '13px', color: 'var(--color-point-main)', minHeight: '18px' }}>
          {error || shippingAddressDetailError}
        </div>
      )}
    </div>
